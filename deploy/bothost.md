...
При старте при необходимости в **`sys.path`** добавляется каталог **`src`**; затем **авто-установка зависимостей**, если нет `fastapi`, а рядом с проектом лежат `requirements.txt` и `pyproject.toml` (типичный случай «запуск из `/app/src` без `pip install»`). Отключить только pip-шаг: **`LINGUABOOST_SKIP_DEPS_BOOTSTRAP=1`**.
